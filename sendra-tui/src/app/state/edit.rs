//! The shared row-edit machinery behind a request's edit session: the
//! cursor-addressable `TextField` primitive every field edit shares,
//! `HeaderRow` (also reused by `super::environment_edit::EnvironmentEditState`
//! for environment variables), and everything `EditState` itself is built
//! from — `EditField`, `BodyEdit`, `AuthEdit`, `AssertionRow`, `CaptureRow`,
//! `JsonOperator`, `CaptureKind` — plus `EditState` itself and the shared
//! `add_row`/`delete_row` helpers its row-based fields (headers, assertions,
//! captures) all build on.

use std::collections::BTreeMap;

use sendra_core::{
    ApiKeyAuth, ApiKeyLocation, Assertions, Auth, BasicAuth, CaptureSource, Captures, Method,
    NotAssertions, OAuthAuth, OAuthGrantType, Request,
};
...
impl AuthEdit {
    ...
    pub(crate) fn to_auth(&self) -> Option<Auth> {
        match self {
            AuthEdit::None => None,
    ...
    pub(crate) fn field_order(&self) -> &'static [AuthField] {
        match self {
            AuthEdit::None => &[],
    ...
    pub(crate) fn text_field_mut(&mut self, field: AuthField) -> Option<&mut TextField> {
        match (self, field) {
    ...
    pub(crate) fn toggle(&mut self, field: AuthField) -> bool {
        match (self, field) {
...
impl JsonOperator {
    ...
    pub(crate) fn wrap(self, arg: serde_json::Value) -> serde_json::Value {
        match self {
    ...
    pub(crate) fn detect(value: &serde_json::Value) -> (Self, serde_json::Value) {
...
pub(crate) fn validate_assertion_value_text(text: &str) -> Result<serde_json::Value, String> {
...
impl EditState {
    ...
    /// arm.
    pub(crate) fn add_header_row(&mut self) {
    ...
    pub(crate) fn delete_focused_header_row(&mut self) {
    ...
    pub(crate) fn add_assertion_row(&mut self) {
    ...
    pub(crate) fn delete_focused_assertion_row(&mut self) {
        let index = match self.focus {
    ...
    pub(crate) fn add_capture_row(&mut self) {
    ...
    pub(crate) fn delete_focused_capture_row(&mut self) {
        let index = match self.focus {
    ...
    pub(crate) fn has_editable_body(&self) -> bool {
    ...
    /// `render_edit_pane`.
    pub(crate) fn auth_field_order(&self) -> &'static [AuthField] {
    ...
    /// `render_edit_pane`.
    pub(crate) fn assertion_row_count(&self) -> usize {
        self.assertions.len()
    }
    ...
    /// `Message::EditFocusNext`/`EditFocusPrev` arms and to
    /// `render_edit_pane`.
    pub(crate) fn capture_row_count(&self) -> usize {
        self.captures.len()
    }
    ...
    pub(crate) fn toggle_focused(&mut self, forward: bool) -> bool {
        match self.focus {
    ...
    pub(crate) fn to_request(&self, base: &Request) -> Request {
    ...
    /// `assertions: {}` block.
    pub(crate) fn to_assertions(&self, base: Option<&Assertions>) -> Option<Assertions> {
    ...
    pub(crate) fn to_captures(&self) -> Option<Captures> {
...
pub(crate) fn validate_method_text(text: &str) -> Result<Method, String> {
